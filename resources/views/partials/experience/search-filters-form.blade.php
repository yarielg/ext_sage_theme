<div class="experience-search container-fluid">
    <div class="container">

        <div class="row mb-4">
            <form class="row mt-1 g-3" method="GET" action="/experience-results">
                {{-- Keyword --}}
                <div class="col-12">
                ...
                <div class="col-md-4">
                    <label for="inputClient" class="form-label">Client Type</label>
                    <select id="inputClient" name="client-type" class="form-select">
                        <option value="" selected>Choose...</option>
                        @foreach( $filter_client_type_options as $term ) 
                            <option 
                                value="{{ $term->term_id }}"
                                @if ( isset( $getvars['client-type'] ) && $getvars['client-type'] == $term->term_id )
                                    selected
                                @endif
                            >
                                {{ $term->name }}
                ...
                <div class="col-12 d-flex justify-content-between align-items-center pt-2">
                    <a href="/experience-results" class="btn btn-lg btn-link text-secondary p-0">View All</a>
